...
export class ReviewMensajeExitoUpdateFamiliar {
    ...
    public static readonly isVisibleSuccessMessage = Question.about(
        'el mensaje de éxito de actualizar familiar',
        async (actor: AnswersQuestions & UsesAbilities) => {

            const successMessage = RegistrarFamiliarPage.MENSAJE_EXITO;
            ...
            await actorInTheSpotlight().attemptsTo(
                Ensure.that(successMessage.text(), equals('¡Los datos fueron actualizados con éxito!'))
            ); 
